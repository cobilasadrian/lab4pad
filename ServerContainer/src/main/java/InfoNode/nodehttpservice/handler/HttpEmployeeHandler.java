...
//HttpRequestHandler reprezinta o rutina de procesare a unui grup specific de cereri HTTP.
//Scopul principal al unui request handler este de a genera un obiect raspuns cu o entitate
// de continut pentru a fi trimis inapoi la client, ca raspuns la cerere
...
public class HttpEmployeeHandler implements HttpRequestHandler {
    ...
    public void handle(HttpRequest request, HttpResponse response, HttpContext context) throws HttpException, IOException {
        ...
        if (method.equals("POST")) {
            try {
                int id = Integer.parseInt(new URIBuilder(request.getRequestLine().getUri())
                ...
                if (infoService.getEmployees().size() > 0 && infoService.getEmployees().size() >= id) {
                    ...
                    if (request instanceof HttpEntityEnclosingRequest) {
                        ...
                        System.out.println("Entity content has been modified: " + infoService.setEmployee(id - 1, e).toString());
        ...
        if (method.equals("GET")) {
            ...
            try {
                ...
                if (infoService.getEmployees().size() > 0 && infoService.getEmployees().size() >= id) {
                    ...
                    ObjectMapper mapper = new ObjectMapper();
                    String e = mapper.writeValueAsString(infoService.getEmployee(id-1));
